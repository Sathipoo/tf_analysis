...
WITH DateParam AS (
    SELECT DATEADD(mi, DATEDIFF(mi, GETDATE(), GETUTCDATE()), CONVERT(datetime, '2025-10-14 12:34:36', 120)) AS FilterTime
)
SELECT 
    CAST(
        'SELECT T.* FROM (' + 
        LEFT(A.test, LEN(A.test)-9) + 
        ') T WHERE T.time_stamp >= ''' + 
        CONVERT(VARCHAR, D.FilterTime, 121) + 
        '''' 
        AS VARCHAR(8000)
    ) AS Param
FROM (
    SELECT STUFF((
        SELECT 
            ' SELECT time_stamp, CS_' + SUBSTRING(table_name,4,4) + '_CS_BGT_AWPD_COMP_ID_ID AS CS_YYYY_CS_BGT_AWPD_COMP_ID_ID, ' +
            'u2_id AS STUDENT_ID, ' +
            SUBSTRING(table_name,4,4) + ' AS ACADEMIC_YEAR ' +
